                    ...
                    >
                        ...
                        >
                            ...
                            <div className="w-full space-y-3">
                                <button className="w-full py-4 bg-primary-container text-white rounded-2xl font-bold text-lg shadow-lg shadow-primary-container/20 active:scale-95 transition-transform" onClick={() => { setShowDisclaimerModal(false); setShowSuccessModal(true); }}>
                                    Confirmar y Pagar
                                </button>
                ...
                {showWaitlistModal && (
                    ...
                    >
                        ...
                        >
                            ...
                            <p className="text-on-surface-variant text-sm leading-relaxed mb-8">
                                Te asignaremos la clase si alguien cancela con un máximo de 1 hora antes de iniciar la clase. Asegúrate que puedes tomar la clase si eso sucede.
                            </p>
                            <div className="w-full space-y-3">
            ...
            <main className="pt-20 pb-28 px-6 space-y-8 max-w-md mx-auto">
                ...
                <section className="space-y-4">
                    <div className="flex justify-between items-end">
                        ...
                        <span className="text-xs text-on-surface-variant font-medium">{isWaitlist ? "0 disponibles" : "5 disponibles hoy"}</span>
                    </div>
                    <div className="grid grid-cols-3 gap-4">
                        {[1, 2, 3, 4, 5].map(m => (
                            <button
                                ...
                                className={`flex flex-col items-center justify-center p-6 rounded-lg shadow-sm transition-all ${isWaitlist ? 'bg-surface-container-low opacity-50' : (selected === m ? 'border-2 border-primary-container bg-white' : 'border-2 border-transparent bg-surface-container-lowest')}`}
                            >
                                <span className="material-symbols-outlined text-3xl mb-2" style={{ fontVariationSettings: (selected === m && !isWaitlist) ? "'FILL' 1" : "'FILL' 0" }}>fitness_center</span>
                                <span className="text-[11px] font-bold">M-0{m}</span>
                                <span className="text-[9px] uppercase tracking-wider text-primary font-black mt-1">{isWaitlist ? 'Ocupado' : 'Disponible'}</span>
                            </button>
                        ))}
                    </div>
                </section>
                ...
                <section className="space-y-4 pb-12">
                    {isWaitlist ? (
                        ...
                        <>
                            <button onClick={() => setShowDisclaimerModal(true)} className="w-full h-16 bg-primary-container text-white rounded-xl font-bold text-lg shadow-lg flex items-center justify-center gap-2 active:scale-95 transition-transform">
                                <span className="material-symbols-outlined">payments</span> Pagar en Línea
                            </button>
                            <button onClick={() => setShowDisclaimerModal(true)} className="w-full h-16 bg-primary-container/15 text-on-primary-container rounded-xl font-bold text-lg flex items-center justify-center gap-2 active:scale-95 transition-transform">
